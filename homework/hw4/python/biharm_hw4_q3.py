...
# not calculating forcing function from manufactured solution anymore. also not calculating u_true since no manufactured solution.
...
# f.interpolate(d4cx * cz + 2.0 * ddcx * ddcz + cx * d4cz) # finds f required to generate manufactured psi
...
# calculating forcing function from given T
# A = 0.1
# f.interpolate( -A*np.pi*np.sin(np.pi*x) )
A = 0.1
...
outfile = VTKFile("result/biharm_hw4_q3.pvd")
outfile.write(T, u.subfunctions[1], v) # Temperature, streamfunction, velocity

#calculate error norms

# no u_true without manufactured solution
